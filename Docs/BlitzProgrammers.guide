...
unpack onto four disks (this is the standard BUM format). For �10.00, registered
...
@REMARK ======================================== 3. TYPES, ARRAYS AND LISTS
...
@{fg shine}NEWTYPE.Alien
    flags.w:x:y
End NEWTYPE

Dim List Aliens.Alien(100)@{fg text}

The difference between a list and an array is that AmiBlitz3 will keep an internal
...
@{b}Dynamic Linked Lists@{ub}
You may set the dimenstion to 0. In this case the needed memory for the data is 
allocated first, if a new dataset is being inserted and can be freed using 
"ClearList Arrayname(),True".

The memory allocation for dynamisc lists uses a pooling mechanism. If Kick3.0+ is 
available the OS-pooling is used, for lower OS-versions an internal pooling mechanism 
is used. The pooling function allocates memory puddles in the size of 2000 bytes each.
...
sprites to be displayed. Currently this can only be achieved using the DisplayAdjust
...
CSIF is of mostly used in macros for checking macro parameters.
...
$fff8  Standard lo-res slice               6
$fff9  Standard hi-res slice               4
$fffa  Lo-res, dual-playfield slice        6
$fffb  Hi-res, dual-playfiled slice        4
$fffc  HAM slice                           6
...
@{b}Pointer Sprite#,Sprite Channel[,Port] @{ub}
...
@{b}MouseArea Minx,Miny,Maxx,Maxy[,Port] @{ub}
